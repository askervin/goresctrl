...
// replacement for the default filesystem interface (fsi).
...
type mockFile struct {
	...
	// User/mock-defined properties
	info *mockFileInfo

	// File-specific overrides for the default file behavior
	open  func(string) (fileIface, error)
	read  func([]byte) (int, error)
	write func([]byte) (int, error)

	// Mock-defined properties
	fs           *fsMock
	filename     string
...
func (mf mockFile) Write(b []byte) (n int, err error) {
	...
	if mf.write != nil {
		...
		if newpos > cap(mf.data) {
			newdata := make([]byte, newpos, newpos)
