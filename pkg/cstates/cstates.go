...
//        cstates.FilterAttrValues(cstates.DISABLE, "1"),
//    )
//    if err != nil {
//        log.Fatal(err)
//    }
...
//        cstates.FilterAttrs(cstates.DISABLE),
//    )
//    if err != nil {
//        log.Fatal(err)
//    }
//    cs.SetAttrs(cstates.DISABLE, "1")
...
type CstateAttribute struct {
	id       AttrID // attribute ID, e.g. RESIDENCY
...
// Cstate{cpu: 2, state: 3}.attrVal[RESIDENCY] corresponds to the file
...
var (
	attributes = []CstateAttribute{
		{id: ABOVE, name: "above", readOnly: true},
		{id: BELOW, name: "below", readOnly: true},
		{id: DISABLE, name: "disable"},
		{id: LATENCY, name: "latency"},
		{id: RESIDENCY, name: "residency"},
		{id: TIME, name: "time", readOnly: true},
	}
...
func NewCstatesFromSysfs(filters ...CstatesFilter) (*Cstates, error) {
	cs := NewCstates()
	cs.fs = NewSysfs()
	if err := cs.populateFromFS(filters...); err != nil {
		return nil, err
	}
	return cs, nil
}

// populateFromFS populates the Cstates instance from filesystem that
// implements the sysfsIface interface.
func (cs *Cstates) populateFromFS(filters ...CstatesFilter) error {
...
func (cs *Cstates) Copy(filters ...CstatesFilter) *Cstates {
	...
	for _, cstate := range cs.cstates {
		...
		if len(filters) > 0 && newCstate.AttrCount() == 0 {
			continue
		}
		newCs.Add(newCstate)
	}
